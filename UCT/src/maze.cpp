...
/*
 * Display a deterministic policy
 * Assume states is ordered by rows and columns, as generated by listStates
 */
void Maze::DisplayPolicy(const vector<State>& states, const vector<int>& actions, std::ostream& ostr) const{
    
    for(int i=0; i < states.size(); i++){
        ostr << "[";
        DisplayAction(actions[i], ostr);
        ostr << "]";
        
        if(states[i].col == cols-1)
            ostr << std::endl;
    }
}
